  ...
  async addTeam(id, nom, ecole, nb, coach, telephone, email) {
    ...
    let myErr = await (async () => {
      ...
      try {
        ...
        const queryText =
          "INSERT INTO Teams(idTeam, nom, ecole, nbJoueurs, coach, telephone, email) VALUES ($1, $2, $3, $4, $5, $6, $7)";
  ...
  async updateTeam(id, nom, ecole, nb, coach, telephone, email) {
    ...
    let myErr = await (async () => {
      ...
      try {
        await client.query("BEGIN");
        const queryText =
          "UPDATE Teams SET nom = $2, ecole = $3, nbJoueurs = $4, coach = $5, telephone = $6, email = $7 WHERE idTeam = $1";
        const queryValues = [id, nom, ecole, nb, coach, telephone, email];
        await client.query(queryText, queryValues, (err, res) => {
          if (err != null) {
            console.log(err);
